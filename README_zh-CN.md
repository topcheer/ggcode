...
- **退出码智能诊断** - 当命令因信号终止时（137=OOM、139=段错误、134=abort、130=SIGINT、143=SIGTERM），Agent 会收到可操作的诊断提示而非裸退出码，实现即时根因分析
- **PTC（程序化工具调用）** — Agent 在沙箱 JS 运行时中编写代码，批量调用只读工具（read_file、grep、git_log、web_search 等），单次往返完成多步分析，大幅降低上下文窗口消耗
- **MCP 集成** — 连接外部工具和数据源
