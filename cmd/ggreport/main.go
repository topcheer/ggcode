...
func main() {
	output := flag.String("o", "ggreport.html", "output HTML file path")
	noOpen := flag.Bool("no-open", false, "do not auto-open the report in browser")
	sessionsDir := flag.String("sessions-dir", "", "override sessions directory (default: ~/.ggcode/sessions)")
	flag.Parse()
	...
	if dir == "" {
		...
		if err != nil {
			fatal("cannot find sessions directory: %v\nIf your sessions are elsewhere, use -sessions-dir flag.", err)
		}
	}

	fmt.Fprintf(os.Stderr, "ggreport: scanning %s ...\n", dir)
	...
	if len(results) == 0 {
		fatal("no sessions found in %s", dir)
	}
	...
	fmt.Fprintf(os.Stderr, "ggreport: wrote %s (%s, %d sessions)\n", *output, sizeStr, len(report.Sessions))
...
func fatal(format string, args ...interface{}) {
	fmt.Fprintf(os.Stderr, "ggreport: "+format+"\n", args...)
	os.Exit(1)
}
