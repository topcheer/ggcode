...
GOMEMLIMIT="${VERIFY_CI_MEMLIMIT:-1GiB}" go build -tags goolm -o /tmp/ggcode ./cmd/ggcode
...
for target in "linux/amd64" "windows/amd64"; do
  os="${target%%/*}"
  arch="${target##*/}"
  if ! CGO_ENABLED=0 GOOS="$os" GOARCH="$arch" GOMEMLIMIT="${VERIFY_CI_MEMLIMIT:-1GiB}" go build -tags goolm ./cmd/ggcode 2>/tmp/cross-build.err; then
...
GOMEMLIMIT="${VERIFY_CI_MEMLIMIT:-1GiB}" go vet -tags goolm ./cmd/... ./internal/...
...
for pkg in $(go list -tags goolm ./cmd/... ./internal/... 2>/dev/null); do
  ...
  if ! GOMEMLIMIT="${VERIFY_CI_MEMLIMIT:-1GiB}" go test -tags goolm -timeout 300s -p 1 -parallel 1 "${pkg}"; then
    test_failed=1
  fi
done
...
if [ -d "${desktop_dir}" ] && [ -f "${desktop_dir}/go.mod" ]; then
  ...
  (cd "${desktop_dir}" && CGO_ENABLED=1 GOMEMLIMIT="${VERIFY_CI_MEMLIMIT:-1GiB}" go test -tags goolm -count=1 -timeout 120s ./...)
fi
