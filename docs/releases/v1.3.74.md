...
- Extpane tabs are automatically created when spawning sub-agents or teammates. No configuration changes required.
- Terminal backend is auto-detected. In nested environments (e.g., iTerm2 inside tmux), tmux takes priority.
- Tabs auto-close when agents finish. Use the `maxPanes=10` safety cap to prevent tab proliferation.
