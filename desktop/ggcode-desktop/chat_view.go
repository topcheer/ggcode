...
type tappableCollapseArea struct {
	widget.BaseWidget
	content fyne.CanvasObject
	onTap   func()
}

func newTappableCollapseArea(content fyne.CanvasObject, onTap func()) *tappableCollapseArea {
	w := &tappableCollapseArea{content: content, onTap: onTap}
	w.ExtendBaseWidget(w)
	return w
}

func (w *tappableCollapseArea) CreateRenderer() fyne.WidgetRenderer {
	return widget.NewSimpleRenderer(w.content)
}

func (w *tappableCollapseArea) Tapped(*fyne.PointEvent) {
	if w.onTap != nil {
		w.onTap()
	}
}
	...
	for _, msg := range messages {
		...
		case "user":
			...
			if len(textParts) > 0 {
				...
				chatMsg := &ChatMessage{Role: "user", Content: joined}
				if w := cv.renderUser(chatMsg); w != nil {
					cv.vbox.Add(w)
					cv.msgWidgets = append(cv.msgWidgets, w)
				}
			}
			...
			for _, block := range msg.Content {
				...
				case "text":
					if strings.TrimSpace(block.Text) != "" {
						chatMsg := &ChatMessage{Role: "assistant", Content: strings.TrimSpace(block.Text)}
						if w := cv.renderAssistant(chatMsg); w != nil {
							cv.vbox.Add(w)
							cv.msgWidgets = append(cv.msgWidgets, w)
						}
					}
