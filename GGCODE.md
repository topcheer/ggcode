...
| Latest documented release | [`v1.3.160`](docs/releases/v1.3.160.md) |
...
- **Sub-agents** (`internal/subagent/`): Manager with semaphore-based concurrency, configurable timeout (default 30 min), progress tracking. Runner executes tasks in isolated agent instances. Sub-agent system prompts include the working directory so agents know their project root without discovery. Manager exposes `CancelAll()` to cancel all running sub-agents at once. **Named subagents** (`template_store.go`): persistent per-workspace templates stored under `~/.ggcode/subagents/<workspace-hash>/`, defining `system_prompt`, `tools` (allowlist+denylist), `mcp_servers`, and optional `model` override. `use_namedagent` clones the provider via `CloneWithModel` for model-specific agents. Available named agents are auto-injected into the system prompt on startup.
...
**Named Subagents** (4, registered in `internal/tui/repl.go`): `create_namedagent`, `use_namedagent`, `list_namedagent`, `delete_namedagent` — define and invoke reusable per-workspace subagent templates with custom system prompts, tool restrictions, MCP bindings, and model overrides
...
- **Autopilot Goal-directed execution**: In autopilot mode, the agent declares a Goal via a `GOAL:` sentinel line at the start of each session. The Goal defines what 'done' looks like. All work must serve the Goal. A strategist process monitors progress and decides when the Goal is achieved (via `GOAL_ACHIEVED`) — the agent itself does not declare completion. Continuation heuristics anchor to the original task to prevent drift.
- **Session persistence — incremental append only**: JSONLStore.Save() only creates the file and updates the index. Messages are persisted exclusively via `AppendMessageToDisk` / `AppendMessagesBatchToDisk` (incremental appends). The old full-rewrite Save() would destroy pre-compaction history after ses.Messages was replaced — this path is now eliminated.
- **Session switch stale references**: `r.model` in REPL is a Bubble Tea value-type copy, not a pointer. After `/clear` or `/sessions` switch, `r.model.Session()` still returns the old session. All runtime handlers (persistHandler, checkpointHandler, exit cleanup, RuntimeStatus) must use `r.getCurrentSession()` (thread-safe pointer updated via `sessionUpdateCallback`) instead of `r.model.Session()`. Similarly, LAN Chat `SetSessionID` is called from `Model.SetSession()` to cover all switch paths, and checkpoint manager is cleared in `switchToSession`.
- **IM binding orphan prevention**: When an empty session is deleted on exit, `ClearSessionBindings` / `ClearSessionBindingsGlobal` removes the `LastSessionID` from all IM adapter bindings referencing that session. Without this, the binding becomes orphaned and no new ggcode instance will auto-claim it.
- **Sub-agent/swarm cancellation on /clear**: `/clear` has a `loading` guard (same as `/sessions` resume) that blocks the switch while the agent is running. It also calls `CancelAll()` for sub-agents and swarm teammates before switching — without this, their results would be routed to the new session via `handleSubAgentDoneMsg`.
