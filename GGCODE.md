...
| Latest documented release | [`v1.3.135`](docs/releases/v1.3.135.md) |
...
- **Context engineering — tool-result + tool-use input clearing**: At 75% of compaction threshold, `ClearOldToolResults` replaces old tool_result Output (>500 chars, non-error) with a placeholder. `ClearOldToolUseInputs` then truncates the matching tool_use Input (>200 chars) to a minimal `{"_cleared":true}` placeholder. Biggest savings from `edit_file` (old_text/new_text), `write_file` (content), `multi_file_edit` (multiple files' content). Pure mechanical — no LLM call needed.
