...
func (c *Client) Prompt(ctx context.Context, prompt string) (*tool.ACPPromptResult, error) {
	result, err := c.promptInternal(ctx, prompt, nil)
	if err != nil {
		return nil, err
	}
	return toolPromptResult(result), nil
}

func (c *Client) PromptStream(
	ctx context.Context,
	prompt string,
	onEvent func(tool.ACPPromptEvent),
) (*tool.ACPPromptResult, error) {
	result, err := c.promptInternal(ctx, prompt, onEvent)
	if err != nil {
		return nil, err
	}
	return toolPromptResult(result), nil
...
func toolPromptResult(result *PromptResult) *tool.ACPPromptResult {
