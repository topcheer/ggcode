...
func TestSlackPanelUnbindRemovesChannel(t *testing.T) {
	...
	m.config.IM.Adapters["sl-test"] = config.IMAdapterConfig{Enabled: true, Platform: "slack"}
	imMgr := im.NewManager()
	store := im.NewMemoryBindingStore()
	if err := imMgr.SetBindingStore(store); err != nil {
		t.Fatalf("SetBindingStore: %v", err)
...
func TestSlackBindingLabelsMutedActiveAvailable(t *testing.T) {
	m := NewModel(nil, nil)
	entries := []slackBindingEntry{
	...
	labels := m.slackBindingLabels(entries)
	if len(labels) != 4 {
