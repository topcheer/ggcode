...
func TestSlackPanelEscClosesPanel(t *testing.T) {
	m := NewModel(nil, nil)
	m.openSlackPanel()
	updated, cmd := m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyEsc})
	if cmd != nil {
...
func TestSlackPanelCreateModeInput(t *testing.T) {
	m := NewModel(nil, nil)
	m.openSlackPanel()
	updated, _ := m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'i'}})
	m = updated
	if !m.slackPanel.createMode {
		t.Fatal("expected create mode to be active")
	}
	m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("sl-test")})
	updated, _ = m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyEsc})
	m = updated
	if m.slackPanel.createMode {
...
func TestSlackPanelUnbindRemovesChannel(t *testing.T) {
	...
	m.SetIMManager(imMgr)
	m.openSlackPanel()
	_, cmd := m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'u'}})
	if cmd == nil {
...
func TestSlackPanelNoEntriesShowsMessage(t *testing.T) {
	m := NewModel(nil, nil)
	m.openSlackPanel()
	updated, _ := m.handleSlackPanelKey(tea.KeyMsg{Type: tea.KeyEnter})
	m = updated
	if m.slackPanel == nil {
