...
func shouldExecuteWhileBusy(text string) bool {
	...
	case "/export-trace",
		...
		"/diff", "/hooks", "/cost", "/commit", "/retry", "/edit", "/copy", "/context", "/prompt", "/regenerate", "/regen", "/cron", "/debug", "/title", "/pin", "/runreport":
		return true
	}
	return false
}
...
func (m *Model) handleCommandWithDisplay(text string, displayInChat bool) tea.Cmd {
	...
	if strings.HasPrefix(text, "/") {
		...
		case "/rules":
			return m.handleRulesCommand()
		case "/undo":
			return m.handleUndoCommand()
		case "/undo-run":
			return m.handleUndoRunCommand()
		case "/files":
		...
		case "/commit":
			return m.handleCommitCommand(parts)
		case "/review":
		...
		case "/prompt":
			return m.handlePromptCommand(parts)
		case "/notify":
