...
func (m Model) renderMatrixPanel() string {
	...
	if panel.createMode {
		body = append(body,
			" "+m.t("panel.matrix.bot_input", maskPositionalCreateEcho(panel.createInput, 2)+"█"),
...
func (m *Model) ensureMatRuntime() error {
	return m.ensureCurrentWorkspaceIMManager(m.t("panel.matrix.error.config_unavailable"), "", true)
}
