...
func (m *Model) handleTmuxCommand(parts []string) tea.Cmd {
	if len(parts) > 1 {
		...
		if sub == "enter" || sub == "attach" || sub == "new" {
			...
			return m.enterTmuxSession(sessionName)
		}
	}
...
func (m *Model) enterTmuxSession(sessionName string) tea.Cmd {
	if m.tmuxClient == nil {
...
func (m *Model) focusTmuxPane(paneID string) {
	...
	defer cancel()
	if err := m.tmuxManagerForWorkspace().Focus(ctx, paneID); err != nil {
...
func (m *Model) captureTmuxPane(paneID string, lines int) {
	...
	defer cancel()
	out, err := m.tmuxManagerForWorkspace().Capture(ctx, paneID, lines)
	if err != nil {
...
func (m *Model) closeTmuxPane(paneID string) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	if err := m.tmuxManagerForWorkspace().Close(ctx, paneID); err != nil {
		m.markTmuxPaneAlive(paneID, false)
