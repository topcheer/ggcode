...
func TestTokenCalibrator_RecordSample_AdjustsRatio(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		c.RecordSample(1000, 1200, 4000, 0) // estimated 1000, actual 1200
	}
	...
	for i := 0; i < 3; i++ {
		c.RecordSample(1000, 1200, 4000, 0)
	}
...
func TestTokenCalibrator_WarmupNoAdjust(t *testing.T) {
	...
	for i := 0; i < calibWarmupSamples; i++ {
		c.RecordSample(500, 2000, 4000, 0) // extreme but should be ignored
	}
	if got := c.ASCIICharsPerToken(); got != 3.5 {
...
func TestTokenCalibrator_RatioClamped(t *testing.T) {
	...
	// Bypass warmup
	for i := 0; i < calibWarmupSamples; i++ {
		c.RecordSample(100, 1, 4000, 0) // extreme: estimated >> actual
	}
	// Force several adjustments
	for i := 0; i < 30; i++ {
		c.RecordSample(100, 1, 4000, 0) // factor = 100, wants to push ratio very high
	}
	...
	for i := 0; i < calibWarmupSamples; i++ {
		c2.RecordSample(1, 10000, 4000, 0)
	}
	for i := 0; i < 30; i++ {
		c2.RecordSample(1, 10000, 4000, 0) // factor = 0.0001, wants to push ratio very low
	}
...
func TestTokenCalibrator_CompositionIsolation(t *testing.T) {
	...
	c2 := NewTokenCalibrator()
	for i := 0; i < 11; i++ {
		c2.RecordSample(2000, 1000, 0, 3000)
	}
...
func TestTokenCalibrator_MixedSampleSplitsFactor(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		pure.RecordSample(1250, 1000, 8000, 0)     // pure ASCII
		mixed.RecordSample(1250, 1000, 4000, 3000) // ~57%/43% mix
	}
