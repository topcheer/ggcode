...
func (a *Agent) RunStreamWithContent(ctx context.Context, content []provider.ContentBlock, onEvent func(provider.StreamEvent)) (err error) {
	...
	for i := 0; a.maxIter <= 0 || i < a.maxIter; i++ {
		...
		// Execute tool calls and build the tool_result message. The per-call
		// machinery (pre-execution detectors, permission, execution, result
		// post-processing) lives in executeToolBatch (tool_exec.go). msgs is
		// write-only inside the batch, so the single resync below replaces the
		// per-Add refreshes the inline code used to do.
		batch := a.executeToolBatch(ctx, onEvent, toolCalls, i, runStats)
		if batch.cancelErr != nil {
			return batch.cancelErr
		}
		toolResults := batch.toolResults
		followUpMessages := batch.followUpMessages
		deferredMemoryContent := batch.deferredMemoryContent
		deferredMemoryFiles := batch.deferredMemoryFiles
		deferredMemoryTarget := batch.deferredMemoryTarget
		msgs = a.contextManager.Messages()
		if err := ctx.Err(); err != nil {
