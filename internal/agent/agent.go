...
func (a *Agent) RunStreamWithContent(ctx context.Context, content []provider.ContentBlock, onEvent func(provider.StreamEvent)) (err error) {
	...
	for i := 0; a.maxIter <= 0 || i < a.maxIter; i++ {
		...
		if len(toolCalls) == 0 {
			stop, stopErr := a.handleNoToolCallResponse(ctx, i, textBuf, resp, toolCalls,
				truncated, policyBlocked, &truncationContinues, &syncVerifyRetries, onEvent,
				&inlineToolCallNudges, &todoCheckCount, &asyncVerifyStats,
				runStats, userPromptForStats)
			if stopErr != nil || stop {
				return stopErr
			}
			continue
		...
		for idx, tc := range toolCalls {
			...
			followUpMessages = append(followUpMessages, a.postExecutionDetectorPass(tc, &result, i, idx, memoHit, runStats, batchConflictWarnings)...)
...
// returned.
// handleNoToolCallResponse handles an iteration whose response contained no tool
// calls — the natural end-of-turn path plus every gate that may extend the run
// instead (truncation auto-continuation, autopilot strategist, unverified-claim /
// sync-verify / adversarial-review / commit-hint gates). It returns (true, err)
// when the whole run must stop with err (nil for normal completion), or
// (false, nil) when the caller must continue the iteration loop.
...
func (a *Agent) handleNoToolCallResponse(ctx context.Context, i int, textBuf string,
	resp *provider.ChatResponse, toolCalls []provider.ToolCallDelta, truncated bool, policyBlocked bool,
	truncationContinues *int, syncVerifyRetries *int, onEvent func(provider.StreamEvent),
	inlineToolCallNudges *int, todoCheckCount *int, asyncVerifyStats **RunStats,
	runStats *RunStats, userPromptForStats string) (stop bool, err error) {
