...
// using incremental appends (no full rewrite). Only messages beyond the
// previously persisted prefix are appended; meta via AppendMetaToDisk.
func SaveSessionMessages(store session.Store, ses *session.Session, messages []provider.Message) error {
	if store == nil || ses == nil {
		return nil
	}
	// The old in-memory slice IS the persisted prefix: every write path
	// appends exactly what it stores in ses.Messages. Keep it to compute
	// the delta below (#1495 case B).
	old := ses.Messages
	...
	if jsonlStore, ok := store.(*session.JSONLStore); ok {
		...
		// #1495 case B: appending the FULL message slice on every save
		// grew the JSONL quadratically over a session's lifetime (each
		// turn re-wrote the entire history) and relied on Load-side dedup
		// to paper over the duplicates. Append only the new tail; if the
		// new slice rewrote history (not a pure extension of old), fall
		// back to the full append — Load-side dedup keeps that correct.
		persisted := persistedPrefixLen(old, messages)
		if persisted < len(messages) {
			return jsonlStore.AppendMessagesBatchToDisk(ses, messages[persisted:])
		}
		return nil
	}
	return nil
...
// persistedPrefixLen returns the length of the common prefix between old
// (the previously persisted messages) and msgs (the new full history).
// Messages are matched by ID when both carry one (Manager.Add assigns
// msg_+ULID); legacy ID-less records fall back to role + first text block.
// A non-matching position truncates the prefix there.
func persistedPrefixLen(old, msgs []provider.Message) int {
	n := len(old)
	if n > len(msgs) {
		n = len(msgs)
	}
	for i := 0; i < n; i++ {
		if old[i].ID != "" && msgs[i].ID != "" {
			if old[i].ID != msgs[i].ID {
				return i
			}
			continue
		}
		if old[i].Role != msgs[i].Role || firstTextOf(old[i]) != firstTextOf(msgs[i]) {
			return i
		}
	}
	return n
...
// firstTextOf returns the first text block of a message ("").
