...
// (SETVALUE, EXEC, PUT) stay uncatchable - a plain substring check on
// short roots would re-flag get_dataset/asset_search (#996 regression).
// readOnlyHint annotations remain the follow-up for full coverage.
...
func TestIssue998AllCapsReadToolsNotBlocked(t *testing.T) {
	allowed := []string{
		...
		"QUERY_ROWS",
		"GET_SETTINGS",
		"TRUNCATED_ROWS", // plain "truncated_rows": "run" only inside word, short root not whole segment
	}
