...
`
	report, err := AnalyzeSource("mem.go", []byte(src), Options{ThresholdComplexity: 1, MaxFunctions: 10})
	if err != nil {
	...
	for _, fn := range report.TopFunctions {
		if fn.Function == "Alpha" {
			found = true
			if fn.File != "mem.go" {
				t.Errorf("File = %q, want mem.go", fn.File)
			}
			if fn.Complexity != 2 {
				t.Errorf("Complexity = %d, want 2", fn.Complexity)
			}
		}
	}
	if !found {
		t.Fatalf("Alpha missing from TopFunctions: %v", report.TopFunctions)
	}

	// nil src must fall back to reading the file from disk.
	dir := t.TempDir()
	path := filepath.Join(dir, "ondisk.go")
	if err := os.WriteFile(path, []byte(src), 0644); err != nil {
		t.Fatal(err)
	}
	report2, err := AnalyzeSource(path, nil, Options{ThresholdComplexity: 1, MaxFunctions: 10})
	if err != nil {
		t.Fatalf("AnalyzeSource(nil src) failed: %v", err)
	}
	if len(report2.TopFunctions) != 1 || report2.TopFunctions[0].Function != "Alpha" {
		t.Fatalf("disk fallback did not parse ondisk.go: %v", report2.TopFunctions)
	}
}
...
func TestIsGenerated(t *testing.T) {
	...
	pb := filepath.Join(dir, "api.pb.go")
	if err := os.WriteFile(pb, []byte("package p\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if !IsGenerated(pb) {
		t.Error("api.pb.go should be detected as generated")
	}

	marker := filepath.Join(dir, "types.go")
	if err := os.WriteFile(marker, []byte("// Code generated by x. DO NOT EDIT.\npackage p\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if !IsGenerated(marker) {
		t.Error("Code generated marker should be detected")
	}

	plain := filepath.Join(dir, "hand.go")
	if err := os.WriteFile(plain, []byte("package p\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if IsGenerated(plain) {
		t.Error("hand-written file must not be detected as generated")
	}
}

func TestAnalyzeSingleFileSkipsGenerated(t *testing.T) {
	dir := t.TempDir()
	pb := filepath.Join(dir, "service.pb.go")
	if err := os.WriteFile(pb, []byte("package p\n\nfunc Gen() int { return 1 }\n"), 0644); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(pb, DefaultOptions())
	if err != nil {
		t.Fatal(err)
	}
	if len(report.TopFunctions) != 0 || report.FilesScanned != 0 {
