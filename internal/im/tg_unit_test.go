...
func TestTGOutboundText(t *testing.T) {
	...
	tests := []struct {
		...
		{"tool_call_edit", OutboundEvent{Kind: OutboundEventToolCall, ToolCall: &ToolCallInfo{ToolName: "edit_file", Args: `{"file_path":"main.go"}`}}, "✏ 编辑文件: `main.go`"},
		...
		{"tool_call_read_svg", OutboundEvent{Kind: OutboundEventToolCall, ToolCall: &ToolCallInfo{ToolName: "read_file", Args: `{"path":"diagram.svg"}`}}, "🖼 读取 SVG: `diagram.svg`"},
		...
		{"tool_result_edit_ok", OutboundEvent{Kind: OutboundEventToolResult, ToolRes: &ToolResultInfo{ToolName: "edit_file", Args: `{"file_path":"main.go","old_text":"foo","new_text":"bar"}`, Result: "ok"}}, "✏ main.go (+1 -1)"},
		{"tool_result_edit_err", OutboundEvent{Kind: OutboundEventToolResult, ToolRes: &ToolResultInfo{ToolName: "edit_file", Args: `{"file_path":"main.go"}`, Result: "conflict", IsError: true}}, "✏ main.go\n```\nconflict\n```"},
...
func TestTGOutboundText_English(t *testing.T) {
	...
	tests := []struct {
		...
		{"tool_call_edit", OutboundEvent{Kind: OutboundEventToolCall, ToolCall: &ToolCallInfo{ToolName: "edit_file", Lang: "en", Args: `{"file_path":"main.go"}`}}, "✏ Edit file: `main.go`"},
